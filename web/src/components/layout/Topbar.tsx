...
export const Topbar: React.FC = () => {
  ...
  return (
    <header className="h-16 bg-white/80 backdrop-blur-xl border-b-2 border-slate-300 shadow-medium flex items-center justify-between px-6 sticky top-0 z-40">
      <div className="flex items-center gap-4">
        <div className="relative">
          <button
            onClick={() => setShowProjectMenu(!showProjectMenu)}
            className="flex items-center gap-2 px-3 py-2 rounded-xl border-2 border-slate-300 bg-white hover:bg-slate-50 hover:border-slate-400 transition-all duration-200 text-sm font-semibold text-slate-700 shadow-sm hover:shadow-md"
          >
          ...
          {showUserMenu && (
            <div className="absolute top-full right-0 mt-2 w-48 bg-white/95 backdrop-blur-xl rounded-2xl border border-slate-200/50 shadow-large z-50 animate-scale-in">
              <div className="p-2">
                ...
                <Link
                  to="/app/profile"
                  className="w-full block px-3 py-2 rounded-lg text-sm text-slate-700 hover:bg-slate-50 transition-colors mt-1"
